...
풀 리퀘스트(Pull Request, PR)는 여러분이 브랜치에서 작업한 내용을 메인 브랜치에 합치고 싶을 때 사용하는 기능입니다. 마치 "제가 이런 기능을 만들었는데, 메인 코드에 합쳐도 될까요?"라고 제안하는 것과 같습니다.
...
깃허브 페이지는 여러분의 HTML, CSS, JavaScript 파일을 웹사이트로 만들어 전 세계에 공개할 수 있게 해주는 아주 멋진 기능입니다. 마치 여러분이 만든 웹페이지를 인터넷에 올리는 것과 같습니다.
