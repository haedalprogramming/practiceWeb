...
1. [조건문 (Conditional Statements)](#1-조건문-conditional-statements)
   - 1-1) [if 문](#if-문)
   - 1-2) [if...else 문](#ifelse-문)
   - 1-3) [if...else if...else 문](#ifelse-ifelse-문)
   - 1-4) [switch 문](#switch-문)

2. [반복문 (Loops)](#2-반복문-loops)
   - 2-1) [for 반복문](#for-반복문)
   - 2-2) [while 반복문](#while-반복문)
   - 2-3) [for...of 반복문 (배열 전용)](#forof-반복문-배열-전용)

3. [반복문 제어 (break, continue)](#3-반복문-제어-break-continue)
   - 3-1) [break](#break)
   - 3-2) [continue](#continue)
...
`switch` 문은 하나의 변수(또는 표현식)의 값에 따라 여러 경우 중 하나를 선택하여 실행할 때 사용합니다. `if...else if...else` 문으로도 같은 기능을 구현할 수 있지만, 특정 값이 명확하게 정해져 있고 그 값에 따라 다른 동작을 해야 할 때 `switch` 문을 사용하면 코드를 더 깔끔하고 읽기 쉽게 만들 수 있습니다.
...
switch (day) {
  case '월요일':
    console.log('오늘은 월요일입니다. 한 주의 시작!');
    break; // 해당 case의 코드를 실행한 후 switch 문을 빠져나갑니다.
  case '수요일':
    console.log('오늘은 수요일입니다. 한 주의 중간!');
    break;
  case '금요일':
    console.log('오늘은 금요일입니다. 주말이 다가옵니다!');
    break;
  default: // 위에 나열된 어떤 case에도 해당하지 않을 때 실행됩니다.
    console.log('오늘은 평일입니다.');
    break;
}
// 출력: 오늘은 월요일입니다. 한 주의 시작!

let fruit = '사과';

switch (fruit) {
  case '사과':
  case '바나나': // '사과' 또는 '바나나'일 경우 동일한 코드를 실행합니다.
    console.log('이것은 과일입니다.');
    break;
  case '당근':
    console.log('이것은 채소입니다.');
    break;
  default:
    console.log('무엇인지 모르겠습니다.');
    break;
}
// 출력: 이것은 과일입니다.
```
...
> [!NOTE]
> `switch` 문은 다음과 같이 구성됩니다:
> *   `switch (표현식)`: 괄호 안의 표현식 값을 평가합니다.
> *   `case 값:`: 표현식의 값이 `값`과 일치하면 이 `case` 블록 안의 코드를 실행합니다.
> *   `break;`: `break` 키워드는 현재 `case` 블록의 실행을 멈추고 `switch` 문을 빠져나가게 합니다. `break`가 없으면 다음 `case` 블록의 코드도 이어서 실행됩니다 (이것을 'fall-through'라고 합니다).
> *   `default:`: 모든 `case`가 일치하지 않을 때 `default` 블록의 코드가 실행됩니다. `default`는 필수가 아닙니다.
...
반복문은 특정 코드 블록을 여러 번 반복해서 실행하도록 할 때 사용합니다. 반복문을 사용하면 똑같은 코드를 여러 번 작성할 필요 없이 효율적으로 작업을 처리할 수 있습니다.
...
`break` 키워드는 현재 실행 중인 반복문을 '즉시 종료'하고, 반복문 다음의 코드로 이동합니다.
...
`continue` 키워드는 현재 반복 단계의 나머지 코드를 '건너뛰고' 다음 반복 단계로 즉시 넘어갑니다.
