...
* **인터프리터 언어**: 컴퓨터가 이해할 수 있는 언어(기계어)로 번역하는 과정에는 크게 두 가지 방식이 있습니다. '컴파일' 방식은 코드를 한 번에 통째로 번역한 후 실행하는 것이고, '인터프리터' 방식은 코드를 한 줄씩 읽어가면서 바로바로 번역하고 실행하는 것입니다. JavaScript는 인터프리터 방식이라서 코드를 작성하고 바로 실행해서 결과를 확인할 수 있어 개발 속도가 빠르고, 작은 수정에도 전체를 다시 번역할 필요가 없습니다. 마치 외국어 통역사가 문장을 듣자마자 바로 통역해주는 것과 비슷합니다.
