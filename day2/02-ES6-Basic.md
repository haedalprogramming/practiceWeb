...
콘솔 출력은 프로그래밍에서 매우 중요한 기능 중 하나입니다. 여러분이 작성한 코드가 제대로 작동하는지 확인하거나, 특정 시점에 변수에 어떤 값이 들어있는지 알아보고 싶을 때 사용합니다. 마치 요리사가 요리 중간에 간을 보거나 재료의 상태를 확인하는 것과 비슷합니다.
...
문(statement)은 JavaScript 코드의 최소 단위로서, 컴퓨터에게 '무엇을 하라'고 지시하는 하나의 명령 또는 작업을 의미합니다. 마치 우리가 일상생활에서 '밥을 먹어라', '물을 마셔라'와 같은 명령을 내리는 것과 같습니다. 모든 JavaScript 프로그램은 이러한 문(statement)들이 모여서 만들어집니다.
...
JavaScript에서 세미콜론(;)은 문(statement)의 끝을 나타내는 역할을 합니다. 마치 문장의 끝에 마침표를 찍는 것과 같습니다. 컴퓨터는 세미콜론을 보고 하나의 명령이 끝났음을 인식하고 다음 명령을 준비합니다.
