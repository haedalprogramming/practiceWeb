...
> [!NOTE]
> Iterator는 데이터를 '소비'하는 방식입니다. 한 번 꺼낸 값은 다시 꺼낼 수 없습니다. 마치 한 번 읽은 책 페이지를 다시 넘기지 않고 다음 페이지로 넘어가는 것과 같습니다.

### 1-2) Generator (제너레이터)

**Generator**는 Iterator를 쉽게 만들 수 있도록 도와주는 특별한 함수입니다. 일반 함수와 달리, Generator 함수는 실행을 중간에 멈췄다가 다시 이어서 실행할 수 있습니다. 마치 영화를 보다가 잠시 멈춤 버튼을 누르고 나중에 다시 이어서 보는 것과 같습니다.
...
**Set**은 중복 없는 유일한 값들만 저장하는 특별한 컬렉션입니다. 마치 여러분이 좋아하는 과일 목록을 만드는데, 같은 과일을 두 번 적지 않는 것과 같습니다.
